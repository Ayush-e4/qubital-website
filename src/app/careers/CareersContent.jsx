        ...
        <AnimateOnScroll className="grid grid-cols-12 gap-6 lg:gap-12 items-center">
          <div className="col-span-12 lg:col-span-7">
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full sm:w-auto">
              <a href="#reach-out" className="inline-flex items-center justify-center h-12 px-6 bg-on-surface text-surface-canvas font-semibold rounded-xl hover:opacity-90 transition-opacity text-sm sm:text-base text-center">
                {c.btn_reach}
              </a>
              <a href="#work-environment" className="inline-flex items-center justify-center h-12 px-6 bg-surface-canvas border border-outline-variant text-on-surface font-semibold rounded-xl hover:bg-surface-card transition-colors text-sm sm:text-base text-center">
                {c.btn_principles}
              </a>
            </div>
          </div>
      ...
      <section id="work-environment" className="container mx-auto px-4 lg:px-8 mb-32 scroll-mt-24">
        ...
        <StaggerContainer className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {(c.benefits ?? []).map((ben, i) => (
            <StaggerItem key={i} className="bg-surface-canvas border border-outline-variant rounded-xl p-6 flex flex-col items-start gap-4 hover:bg-surface-card transition-colors">
      ...
      <section id="reach-out" className="container mx-auto px-4 lg:px-8 mb-16 scroll-mt-24">
        <AnimateOnScroll className="max-w-3xl mx-auto bg-surface-canvas border border-outline-variant rounded-2xl p-6 md:p-7 text-center flex flex-col items-center">
          <div className="flex items-center justify-center gap-2 mb-2.5 text-primary">
            ...
            <span className="text-xs font-mono tracking-wider uppercase text-secondary">{c.network_tag}</span>
          </div>
          <h3 className="text-xl md:text-2xl font-medium text-on-surface mb-2">
            {c.network_title}
          </h3>
          <p className="text-secondary text-sm leading-relaxed max-w-xl mb-5">
            {c.network_desc}
          </p>
          <a href="mailto:contact@qubital.eu?subject=Spontaneous%20Application%20-%20Qubital" className="inline-flex items-center justify-center h-10 px-6 text-sm bg-on-surface text-surface-canvas font-medium rounded-lg hover:opacity-90 transition-opacity">
            {c.network_email}
          </a>
        </AnimateOnScroll>
      </section>
