        ...
        <StaggerContainer className="grid grid-cols-1 lg:grid-cols-12 gap-4 h-auto lg:h-[450px]">
          ...
          <StaggerItem className="lg:col-span-4 bg-surface-card border border-outline-variant/60 rounded-2xl p-6 sm:p-8 flex flex-col justify-between h-auto lg:h-full relative overflow-hidden group">
            ...
            <div className="flex flex-col gap-4 sm:gap-6 relative z-10">
              ...
              <div>
                <h3 className="text-lg sm:text-xl text-on-surface font-semibold mb-2">{a.focus_title}</h3>
      ...
      <section className="w-full max-w-7xl px-4 sm:px-6 lg:px-8 py-10 md:py-16">
        <AnimateOnScroll>
          <div className="flex flex-col gap-4 mb-6 sm:mb-8">
            <h2 className="text-xs font-mono tracking-widest text-secondary uppercase font-semibold">{a.mission_tag}</h2>
          </div>
          ...
          <div className="bg-primary/10 border border-primary/20 border-l-4 border-l-primary rounded-r-xl p-5 sm:p-8 md:p-12 relative overflow-hidden">
            ...
            <div className="relative z-10 max-w-4xl">
              <span className="material-symbols-outlined text-primary text-3xl mb-6">format_quote</span>
              ...
              <div className="flex flex-wrap gap-3">
                {(a.mission_badges ?? []).map((badge, i) => (
                  <span key={i} className="px-3 py-1.5 bg-surface-card border border-outline-variant rounded-lg text-sm text-secondary flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8">
          ...
          <div className="lg:col-span-4 relative">
            <AnimateOnScroll className="lg:sticky lg:top-32 flex flex-col gap-6">
              <div className="text-xs font-mono text-secondary pb-4 border-b border-outline-variant">{a.origins_tag}</div>
          ...
          <div className="lg:col-span-8">
            <AnimateOnScroll>
              <div className="prose prose-invert prose-zinc max-w-none">
                ...
                <h4 className="text-xl font-light text-on-surface mb-6 border-b border-outline-variant pb-4">{a.principles_title}</h4>
              ...
              <StaggerContainer className="grid grid-cols-2 gap-4">
                {(a.stats ?? []).map((stat, i) => (
                  <StaggerItem key={i} className="bg-surface-card border border-outline-variant/60 rounded-xl p-6 flex flex-col gap-2">
      ...
      <section className="w-full max-w-7xl px-4 sm:px-6 lg:px-8 py-16 md:py-24 border-t border-outline-variant/60">
        <AnimateOnScroll>
          <div className="flex flex-col gap-2 mb-12">
            <h2 className="text-xs font-mono tracking-widest text-secondary uppercase">{a.diff_tag}</h2>
            <h3 className="text-3xl font-light text-on-surface">{a.diff_title}</h3>
          </div>
          
          <StaggerContainer className="flex flex-col gap-4">
            {(a.diff_items ?? []).map((diff, i) => (
              <StaggerItem key={i} className="bg-surface-card border border-outline-variant rounded-xl p-6 md:p-8 hover:border-primary/40 hover:shadow-md transition-all group relative overflow-hidden">
                <div className="flex flex-col sm:flex-row items-center sm:items-center gap-6 md:gap-8 relative z-10">
                  ...
                  <div>
                    <h4 className="text-xl text-on-surface font-medium mb-2 text-center sm:text-left">{diff.title}</h4>
