  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none z-20 flex transform-gpu flex-col gap-2 p-5 sm:p-6 md:p-8 transition-all duration-300 md:group-hover:-translate-y-3">
        ...
        <p className="max-w-lg text-secondary text-sm md:text-base leading-relaxed">{description}</p>
      </div>
      ...
      >
        <Link href={href} className="inline-flex items-center gap-2 text-primary font-semibold group/link text-sm uppercase tracking-wider py-1 hover:underline">
          <span>{cta}</span>
          <span className="material-symbols-outlined text-[16px] transition-transform duration-200 group-hover/link:translate-x-1">arrow_forward</span>
        </Link>
      </div>
    </div>
  );
